<template lang="html">
    <div class="mdl-shadow--2dp mdl-color--white mdl-cell mdl-cell--4-col mdl-cell--12-col-tablet mdl-cell--12-col-desktop search-container">
        ...
        <div class="search-instructions" v-if="privateState.results.metric.length === 0 && privateState.results.NSA.length === 0 &&  privateState.results.neighborhood.length === 0 &&  privateState.results.zipcode.length === 0 && privateState.results.address.length === 0">
            Enter a <span class="tooltip" v-bind:title="privateState.neighborhoodDefinition">{{ privateState.neighborhoodDescriptor }}</span>, address, NSA or zip code.
        </div>
  ...
  methods: {
    ...
    searchMetric: function(query) {
      if (query.length >= 3 && !isNumeric(query)) {
        ...
        for (k in dataConfig) {
          if (
            dataConfig[k].title.toUpperCase().indexOf(query.toUpperCase()) !==
            -1
          ) {
            _this.privateState.results.metric.push({
              label: dataConfig[k].title,
              select: dataConfig[k].metric
            });
          }
        }
      } else {
        ...
        axios
          ...
          .then(function(response) {
            ...
            if (response.data.length > 0) {
              const nsas = [
                ...new Set(response.data.map(item => item.nsa_name))
              ];
              nsas.forEach(function(nsa) {
                let npa_items = response.data.filter(
                  item => item.nsa_name === nsa
                );
                let npas = [...new Set(npa_items.map(item => item.id))];
                _this.privateState.results.NSA.push({
                  label: nsa,
                  select: npas.map(String)
                });
              });
            }
          });
        ...
        axios
          ...
          .then(function(response) {
            ...
            if (response.data.length > 0) {
              ...
              _this.privateState.results.zipcode.push({
                label: query,
                select: selected
              });
            }
    ...
    searchNeighborhood: function(query) {
      ...
      if (query.length > 0) {
        ...
        if (match !== -1) {
          this.privateState.results.neighborhood.push({
            label: keys[match],
            select: [keys[match]]
          });
        }
      ...
      if (query.length > 4 && !isNumeric(query)) {
        ...
        // TODO: Update Query create address search
          ...
          .then(function(response) {
            ...
            if (response.data.length > 0) {
              for (let i = 0; i < response.data.length; i++) {
                _this.privateState.results.address.push({
    ...
    selectNeighborhoods: function(n) {
      this.clearResults();
      let selected = [...new Set(n.concat(this.sharedState.selected))];
      this.sharedState.selected = selected;
      this.sharedState.zoomNeighborhoods = selected.slice(0); // this hack is to keep selected and zoomNeighborhoods from linking
    },
      ...
      axios
        ...
        .then(function(response) {
          if (response.data.length === 1) {
            let selected = [
              ...new Set(
                [`${response.data[0].id}`].concat(_this.sharedState.selected)
              )
            ];
            _this.sharedState.selected = selected;
            _this.sharedState.zoomNeighborhoods = selected.slice(0);
